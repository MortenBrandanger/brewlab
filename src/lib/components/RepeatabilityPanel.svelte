<script lang="ts">
	...
	import { variance } from '$lib/state/variance.svelte';

	/*
	 * The 240 runs happen in a worker (see variance.svelte.ts). This effect
	 * re-requests whenever the recipe changes; the panel keeps showing the last
	 * report until the new one arrives, so nothing flickers to empty.
	 */
	$effect(() => {
		variance.request(brew.recipe);
	});
	const report = $derived(variance.report);
...
<section>
	...
	{#if !report}
		<p class="prose-measure text-sm text-muted" aria-live="polite">
			Brewing this beer 240 more times, missing the mash temperature, the efficiency, the volumes
			and the fermentation by as much as a real brew day usually does…
		</p>
	{:else}
		...
		<div class="mt-4">
			...
			{#if top && top.weight > 0.3}
				<p class="prose-measure mt-2 text-xs text-muted">
					...
					<span class="text-fg">{top.label.toLowerCase()}</span>. That is the one worth getting
					right, and the rest matters far less than it feels like it should.
				</p>
			{/if}
		</div>
	{/if}
