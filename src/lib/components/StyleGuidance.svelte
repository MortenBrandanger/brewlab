<script lang="ts">
	...
	import type { Reveal } from '$lib/state/stages';

	/**
	 * A row only means something once the brew day has produced the figure it
	 * shows. Scoring an unbrewed beer against a style — five red markers and
	 * "18% conformity" before the water is even hot — is the same defect as
	 * claiming an ABV before fermentation.
	 */
	const ROW_REVEAL: Record<string, Reveal> = {
		og: 'gravity',
		fg: 'alcohol',
		abv: 'alcohol',
		ibu: 'bitterness',
	...
	const rows = $derived(allRows.filter((row) => brew.knows(ROW_REVEAL[row.key])));
...
{#if style}
	<section class="panel p-4">
		<div class="flex flex-wrap items-baseline justify-between gap-2">
			...
			{#if conformity && rows.length === allRows.length}
		...
		{#if rows.length === 0}
			...
			<dl class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
				{#each allRows as row (row.key)}
					<div>
						<dt class="text-xs text-subtle">{row.label}</dt>
						<dd class="tnum text-sm font-medium">
							{row.format(row.range.min)}–{row.format(row.range.max)}
						</dd>
					</div>
				{/each}
			</dl>
			<p class="prose-measure mt-3 text-xs text-subtle">
				What this beer asks for. Each figure fills in as the brew day reaches the stage that
				produces it.
			</p>
		{:else}
